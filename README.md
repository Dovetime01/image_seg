...
当前实现已能覆盖常见审图与导出流程，下列方向仍有持续改进空间：

1. **大规模组件拆分耗时**  
   组件数量多、图纸分辨率高时，手动拆分及叠色刷新的处理时间会略长。后续可继续做 ROI 局部重绘、异步任务队列或更轻量的预览编码，缩短大图交互等待。

2. **最外层边框清理的误伤**  
   去除图纸最外圈边框时，靠近图框边缘的有效信息（尺寸、注记、局部图元等）有可能被一并清掉。后续可更精细地区分「纯图框线」与「贴边有效内容」，并提供可调边距 / 保护带，降低误删。
